...
def validate_personality_description(description: str) -> Tuple[bool, List[str]]:
    ...
    for pattern in suspicious_patterns:
        if re.search(pattern, sanitized, re.IGNORECASE):
            errors.append("Personality description contains invalid content")
            break
...
def validate_personality_name(name: str) -> Tuple[bool, List[str]]:
    ...
    if not re.match(r'^[a-zA-Z0-9\s\-\.\,\'\"]+$', sanitized):
        errors.append("Personality name contains invalid characters")
...
def validate_confidence_score(confidence: float) -> Tuple[bool, List[str]]:
    """
...
def is_valid_timestamp(timestamp: datetime) -> bool:
    ...
    # Allow timestamps up to 1 minute in the future to account for clock skew
    future_threshold = datetime.now().timestamp() + 60
    return timestamp.timestamp() <= future_threshold
